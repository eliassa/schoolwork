...
public:
  ...
  public:
    ...
    std::string type() const override{
     ...
     bool valid_move(int from_x, int from_y, int to_x, int to_y) const override{
       //burde jeg teste om vi går utenfor brettet?
  ...
  class Knight : public Piece {
    ...
    public:
      ...
      bool valid_move(int from_x, int from_y, int to_x, int to_y) const override{
        //burde jeg teste om vi er innenfor brettet?
        double dist = sqrt(pow((from_x-to_x),2)+pow((from_y-to_y),2));
...
public:
  ...
  void piece_move_invalid (const ChessBoard::Piece &piece, const string &from, const string &to) const{
    cout << "can not move " << piece.type() << " from " << from << " to " << to << endl;
  };
  
  void piece_move_missing(const string &square) const{
    cout << "no piece at " << square << endl;
  };
  
  void after_move() const{
      string res = "_________________________________________________________________________________________________________________________________";
      char bok = 'A';
      for (const auto & indre:board->squares){
        res +="\n";
        res+=bok++;
        res+=+"|\t";
           for (const auto & brikke:indre){
             if ( brikke == nullptr){
               res+= "  0\t";
             }
             else {
...
int main() {
  ...
  board.on_piece_removed = [&print](const ChessBoard::Piece &piece, const string &square) {
    print.piece_removed(piece,square);
  };
  ...
  board.on_piece_move_invalid = [print](const ChessBoard::Piece &piece, const string &from, const string &to) {
    print.piece_move_invalid(piece,from,to);
  };
  board.on_piece_move_missing=[print](const string &square) {
    print.piece_move_missing(square);
  };
